...
def train_iteration(agent, env, min_update_samples=2000, max_episode_steps=200, seed=None) -> dict:
    # Run actual training
...
# Training
def train(cfg_path, cfg_args=None):
    global L
    if cfg_args is None:
        cfg_args = {}
    ...
    for ep in range(cfg.train_episodes + 1):
        train_info = train_iteration(agent, env, min_update_samples=cfg.min_update_samples,
                                     max_episode_steps=cfg.max_episode_steps, seed=cfg.seed)
        train_info.update({'episodes': ep})

        if not cfg.silent:
            print(
                f"Episode {ep} finished. Total reward: {train_info['ep_reward']} ({train_info['timesteps']} timesteps)")
...
def test(episodes, cfg_path, cfg_args=None):
    if cfg_args is None:
        cfg_args = {}
